...
**벨베데레는 마지막 입장이 17:30** 이라 17:00 입장으로는 한 시간밖에 못 본다 →
**12/27 을 재구성했다**: 10:00 쇤브룬(오픈런 부담 완화) → 12:00 마켓·점심 → 13:30 정원·글로리에테 →
**15:00 벨베데레**(두 시간 확보) → 17:30 카페. 벨베데레·오페라·숙소가 모두 트램 D 한 노선이라 오후가 한 줄로 이어진다.
쇤브룬은 겨울 08:30 개장이라 10:00 도 이르지 않고, 일몰 16:05 전에 정원까지 다 본다.
